...
apiClient.interceptors.response.use(
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <header class="h-16 bg-slate-900/80 backdrop-blur border-b border-slate-800 flex items-center justify-between px-8 shrink-0">
        <div class="flex items-center gap-4">
          ...
          <span class="text-xs text-slate-400 font-mono">DB: ${config.database.toUpperCase()}</span>
        </div>
  ...
  <div class="space-y-8 max-w-7xl mx-auto">
    <!-- Header Hero -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-slate-900 to-teal-950/40 p-6 rounded-2xl border border-slate-800">
      <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${Math.min(config.entities.length + 1, 4)} gap-5">
      ...
      return `<div class="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition shadow-sm">
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition shadow-sm">
        <div class="flex items-center justify-between">
...
program2.command("create [name]", { isDefault: true }).description("Generate a new web application template").option("-b, --backend <dotnet|nestjs>", "Backend framework: dotnet or nestjs").option("-f, --frontend <vue|react>", "Frontend framework: vue or react").option("-d, --database <postgres|mysql|sqlserver>", "Database engine: postgres, mysql, sqlserver").option("-p, --preset <presetId>", "Use a preset template: ecommerce, crm, clinic, blog").option("-o, --output <dir>", "Output destination folder").option("--ai <prompt>", 'Prompt for AI to draft schema (e.g. "pet clinic with appointments")').option("--no-auth", "Disable JWT authentication").action(async (name, options) => {
  ...
  const projectName = name || options.output || (await (0, import_prompts.default)({
    type: "text",
    name: "val",
    message: "Project name:",
    initial: "MyEnterpriseApp"
  })).val || "MyEnterpriseApp";
  const backend = options.backend || (await (0, import_prompts.default)({
    type: "select",
    name: "val",
  ...
  const frontend = options.frontend || (await (0, import_prompts.default)({
    type: "select",
    name: "val",
  ...
  const database = options.database || (await (0, import_prompts.default)({
    type: "select",
    name: "val",
...
program2.command("list").description("List all available preset templates").action(() => {
...
async function scaffoldTemplate(config, targetDir) {
  ...
  const resolvedDir = import_path.default.resolve(process.cwd(), targetDir);
  import_fs.default.mkdirSync(resolvedDir, { recursive: true });
  for (const file of files) {
    const fullPath = import_path.default.join(resolvedDir, file.path);
    import_fs.default.mkdirSync(import_path.default.dirname(fullPath), { recursive: true });
    import_fs.default.writeFileSync(fullPath, file.content, "utf8");
  }
  console.log(`
